...
const jsPath   = path.join(repoRoot, 'web', 'real-estate-app.js');
...
if (!fs.existsSync(htmlPath)) { console.error(`Missing file: ${htmlPath}`); process.exit(1); }
if (!fs.existsSync(robotsPath)) { console.error(`Missing file: ${robotsPath}`); process.exit(1); }
...
if (api) {
  ...
  js   = js.split(apiPlaceholder).join(api);
...
if (publicUrl) {
  ...
  js   = js.split(publicPlaceholder).join(publicUrl);
...
if (js) fs.writeFileSync(jsPath, js);
